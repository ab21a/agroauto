...
def tractor_route(model,H,W):
    X = np.zeros((H*W,3))
    k=0
    for r in range(H):
        for c in range(W):
            X[k]=[r/max(1,H-1), c/max(1,W-1), r%2]
            k+=1
    A = np.argmax(model.predict(X,verbose=0),axis=1).reshape(H,W)

    r=c=0
    path=[(r,c)]
    for _ in range(H*W+H):
        if r==H-1 and ((r%2==0 and c==W-1) or (r%2==1 and c==0)):
            break
        a=A[r,c]
        pr,pc=r,c
        if a==1 and r<H-1: r+=1
        elif a==3 and c<W-1: c+=1
        elif a==2 and c>0: c-=1
        if (r,c)==(pr,pc):
            if pr%2==0:
                c=pc+1 if pc<W-1 else pc
                r=pr+1 if pc==W-1 else pr
            else:
                c=pc-1 if pc>0 else pc
                r=pr+1 if pc==0 else pr
        path.append((r,c))
    return path
...
if st.button("Сгенерировать"):
    ...
    p = tractor_route(tractor_model,H,W)
